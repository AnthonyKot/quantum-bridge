...
for(const width of [1440,390]) {
 ...
 for(const route of routes) {
  ...
  const data=await page.evaluate(()=>({
   ...
   answers:document.querySelectorAll('details').length,
...
const printedAnswers = await page.evaluate(() => {
 ...
 const count=document.querySelectorAll('details[open]').length;
...
console.log(JSON.stringify({routes:routes.length,viewports:2,formulas,answers,issues},null,2));
await browser.close();
